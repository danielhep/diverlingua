        ...
        >
          ...
          <div class="flex flex-col space-y-5">
            <span class="flex items-center justify-center space-x-2">
              ...
              <span class="font-normal text-gray-500">or create an account</span>
              <span class="h-px bg-gray-400 w-14" />
            </span>
            <div class="flex flex-col space-y-4">
              ...
              >
                <span class="text-sm font-medium text-gray-800 group-hover:text-white">Sign Up</span>
              </a>
            </div>
          </div>
        </form>
...
export default {
  setup () {
    ...
    const signIn = async () => {
      try {
        ...
        if (e.code === 'auth/wrong-password') {
          error.value = 'Your password was incorrect.'
        } else if (e.code === 'auth/invalid-email') {
          error.value = 'Invalid email address.'
        } else if (e.code === 'auth/user-not-found') {
          error.value = 'No account with this email exists.' // TODO: Make account?
    ...
    return {
      email, password, signIn, error, firebase
    }
  }
}
</script>
